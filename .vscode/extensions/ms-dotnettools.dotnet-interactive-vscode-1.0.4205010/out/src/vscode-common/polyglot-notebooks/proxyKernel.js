                    ...
                    if (connection.isKernelEventEnvelope(envelope)) {
                        if (envelope.eventType === contracts.KernelInfoProducedType &&
                            ...
                            try {
                                ...
                                logger_1.Logger.default.info(`proxy name=${this.name}[local uri:${this.kernelInfo.uri}, command routingSlip :${original}] has changed to: ${JSON.stringify((_d = commandInvocation.commandEnvelope.routingSlip) !== null && _d !== void 0 ? _d : [])}`);
                            }
                            catch (e) {

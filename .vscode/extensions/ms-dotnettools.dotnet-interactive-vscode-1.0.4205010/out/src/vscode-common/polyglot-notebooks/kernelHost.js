...
class KernelHost {
    ...
    connect() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this._defaultConnector.sender.send({ eventType: contracts.KernelReadyType, event: kernekReady, routingSlip: [this._kernel.kernelInfo.uri] });
            return kernekReady;
        });
    }
    getKernelInfos() {
    ...
    getKernelInfoProduced() {
        let events = Array.from(this.getKernelInfos().map(kernelInfo => ({ eventType: contracts.KernelInfoProducedType, event: { kernelInfo: kernelInfo }, routingSlip: [kernelInfo.uri] })));
        return events;
    }
}
